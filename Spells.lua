...
--]]
	...
	[6262] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "minor_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- minor 100
	[23468] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "minor_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- minor 110
	[23469] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "minor_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- minor 120
	[6263] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "lesser_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- lesser 250
	[23470] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "lesser_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- lesser 275
	[23471] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "lesser_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- lesser 300
	[5720] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "hs_used", Result = true, Cooldown = 120, Group = 2, },      -- 500
	[23472] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "hs_used", Result = true, Cooldown = 120, Group = 2, },      -- 550
	[23473] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "hs_used", Result = true, Cooldown = 120, Group = 2, },      -- 600
	[5723] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "greater_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- greater 800
	[23474] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "greater_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- greater 880
	[23475] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "greater_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- greater 960
	[11732] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "major_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- major 1200
	[23476] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "major_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- major 1320
	[23477] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "major_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- major 1440
	[27235] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "master_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- master 2080
	[27236] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "master_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- master 2288
	[27237] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "master_hs_used", Result = true, Cooldown = 120, Group = 2, }, -- master 2496

	-- Spell stone
	[128] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "spellstone_used", Result = true, Cooldown = 180, Group = 3, },
	[17729] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "greater_spellstone_used", Result = true, Cooldown = 180, Group = 3, }, 
	[17730] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "major_spellstone_used", Result = true, Cooldown = 180, Group = 3, },
	[28170] = { UsageRank = 1, SpellRank = 1, Timer = true, Usage = "master_spellstone_used", Result = true, Cooldown = 180, Group = 3, }, 
	...
	destroy_shards = {
		...
		func = function(self, arg1)
			if arg1 == "RightButton" then
				-- Deleting items is protected during combat (causes ADDON_ACTION_BLOCKED)
				if InCombatLockdown and InCombatLockdown() then
					if UIErrorsFrame and UIErrorsFrame.AddMessage then
						UIErrorsFrame:AddMessage("Can't destroy shards in combat.", 1, 0, 0)
					end
					return
				end

				for i = 1, C_Item.GetItemCount(Necrosis.Warlock_Lists.reagents.soul_shard.id) do
...
Necrosis.Warlock_Lists = {
	...
	["spell_stones"] = {
		-- TBC Classic / Anniversary (equippable spellstones)
		spell               = { id = 5522, }, -- Spellstone
	...
	["fire_stones"] = {
		...
		lesser_fire        = { id = 1254, }, -- Lesser Firestone
		fire               = { id = 13699, }, -- Firestone
...
-- Updates the spell timer option.
-- When disabling: immediately removes any active displayed timers for that Usage.
-- When enabling: immediately recreates timers if the aura/debuff is already active.
function Necrosis.UpdateSpellTimer(index, show)
	-- Save option
	NecrosisConfig.Timers[index].show = show
	local usage = NecrosisConfig.Timers[index].usage

	-- Apply to spell database
	for i, v in pairs(Necrosis.Warlock_Spells) do
		if Necrosis.Warlock_Spells[i].Usage == usage then
			Necrosis.Warlock_Spells[i].Timer = show
		end
	end

	-- Disable => remove any currently displayed timers for this usage
	if not show then
		local Timer = Necrosis.TimerManagement
		if Timer and Timer.SpellTimer then
			for t = #Timer.SpellTimer, 1, -1 do
				if Timer.SpellTimer[t] and Timer.SpellTimer[t].Usage == usage then
					Timer = Necrosis:RetraitTimerParIndex(t, Timer, "Option Disable")
				end
			end
		end

	-- Enable => rescan current auras and recreate timers immediately if needed
	else
		if Necrosis.RescanActiveTimersForUsage then
			Necrosis:RescanActiveTimersForUsage(usage)
		end
	end
end
...
-- True when the spell name corresponds to "using" a healthstone.
-- These spells are defined in Warlock_Spells as *_hs_used with Cooldown = 120.
function Necrosis.IsSpellHealthStoneUsed(name)
	if name == (Necrosis.Warlock_Spell_Use["minor_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["minor_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["lesser_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["lesser_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["greater_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["greater_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["major_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["major_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["master_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["master_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["demonic_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["demonic_hs_used"]].Name or "")
		or name == (Necrosis.Warlock_Spell_Use["fel_hs_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["fel_hs_used"]].Name or "")
	then
		return true
	else
		return false
...
function Necrosis:SpellSetup(reason)
	...
	for id, v in pairs(Necrosis.Warlock_Spells) do
		...
		Necrosis.Warlock_Spells[id].ID =
		id                            -- redundant to the 'id' but allows the spell to be self contained if returned as a table
