    ...
    elseif (type(glowID) == "string") then
        if (not SAO.RegisteredGlowSpellNames[glowID]) then
            ...
            for _, glowSpellID in ipairs(glowSpellIDs) do
                ...
                -- AwakeButtonsBySpellID does not exist in TBC Classic
                if self.AwakeButtonsBySpellID then
...
-- TBC Classic does not have a global ActionButton_Update; hook what exists.
if type(ActionButton_Update) == "function" then
	hooksecurefunc("ActionButton_Update", HookActionButton_Update);
elseif type(ActionButton_UpdateUsable) == "function" then
	hooksecurefunc("ActionButton_UpdateUsable", HookActionButton_Update);
elseif type(ActionButton_UpdateState) == "function" then
	hooksecurefunc("ActionButton_UpdateState", HookActionButton_Update);
elseif type(ActionButton_UpdateCooldown) == "function" then
	hooksecurefunc("ActionButton_UpdateCooldown", HookActionButton_Update);
end
...
-- TBC Classic does not have StanceBar_UpdateState
