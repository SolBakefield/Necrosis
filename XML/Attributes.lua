...
function Necrosis:MenuAttribute(menu)
    ...
    -- FIX: If state says "Ouvert" but all children are hidden,
    -- treat it as closed so the next click opens in ONE click.
    if Etat == "Ouvert" then
...
function Necrosis:SetPetSpellAttribute(button)
	...
	if f then
		...
		if f.pet then
			...
			-- Get the localized summon spell name. If Necrosis' mapping is missing, fall back to explicit TBC names.
			local castName = Necrosis.GetSpellCastName(f.high_of)

			if castName then
				castName = castName:gsub("%s*%b()", ""):gsub("%s+$", "")
			end

			f:SetAttribute("type1", "spell")
			f:SetAttribute("spell1", castName)
...
function Necrosis:StoneAttribute(Steed)
	...
	if Steed and f then
		-- Helper: set one click slot to either a spell or an item depending on saved ID
		local function SetMountAttr(typeKey, spellKey, itemKey, mountId, defaultSpellName)
			if mountId then
				local spellName = GetSpellInfo(mountId)

				if spellName then
					-- Spell mount (dragged from spellbook)
					f:SetAttribute(typeKey, "spell")
					f:SetAttribute(spellKey, spellName)
					f:SetAttribute(itemKey, nil)
				else
					-- Item mount (dragged from bags)
					local itemName = GetItemInfo(mountId)
					if itemName then
						f:SetAttribute(typeKey, "item")
						f:SetAttribute(itemKey, itemName)
						f:SetAttribute(spellKey, nil)
					else
						-- Bad/unknown ID; clear
						f:SetAttribute(typeKey, nil)
						f:SetAttribute(spellKey, nil)
						f:SetAttribute(itemKey, nil)
					end
				end
			else
				-- No custom mount: use default spell name
				if defaultSpellName then
					f:SetAttribute(typeKey, "spell")
					f:SetAttribute(spellKey, defaultSpellName)
					f:SetAttribute(itemKey, nil)
				else
					f:SetAttribute(typeKey, nil)
					f:SetAttribute(spellKey, nil)
		...
		-- Defaults (Felsteed / Dreadsteed) if available
		local srank1       = self.Warlock_Spells[5784] and self.Warlock_Spells[5784].InSpellBook
		local srank2       = self.Warlock_Spells[23161] and self.Warlock_Spells[23161].InSpellBook
		local Rank1        = srank1 and self.Warlock_Spells[5784].CastName
		local Rank2        = srank2 and self.Warlock_Spells[23161].CastName

		local defaultLeft  = Rank2 or Rank1
		local defaultRight = Rank1 or Rank2

		-- Normal clicks
		SetMountAttr("type1", "spell1", "item1", NecrosisConfig.LeftMount, defaultLeft)
		SetMountAttr("type2", "spell2", "item2", NecrosisConfig.RightMount, defaultRight)

		-- Ctrl-modified clicks (keep same mapping your file already uses)
		SetMountAttr("ctrl-type1", "ctrl-spell1", "ctrl-item1", NecrosisConfig.CtrlRightMount, defaultLeft)
		SetMountAttr("ctrl-type2", "ctrl-spell2", "ctrl-item2", NecrosisConfig.CtrlLeftMount, defaultRight)
	end

	-- Prefer special hearth items over normal Hearthstone:
	-- Naaru's Embrace (260221) > Dark Portal (184871) > Hearthstone
	local function GetBestHearthItem()
		-- Use item:ID so we don't depend on localized names being cached
		if C_Item.GetItemCount(260221) > 0 then
			return "Naaru's Embrace"
		end
		if C_Item.GetItemCount(184871) > 0 then
			return "Dark Portal"
		end
		return self.Translation.Item.Hearthstone
	end
	...
	if f then
		...
		f:SetAttribute("type1", "macro")
		f:SetAttribute("macrotext1", "/necrosis timer")
		f:SetAttribute("type2", "item")
		f:SetAttribute("item2", GetBestHearthItem())
	end
	---[[
	...
	-- 	f:SetAttribute("unit1", "target")
	-- 	f:SetAttribute("type1", "macro")
	-- 	f:SetAttribute("macrotext1", "/necrosis timer2")
	-- 	f:SetAttribute("type2", "")
	-- 	f:SetAttribute("item2", GetBestHearthItem)
	-- end
	--]]
end
...
function Necrosis:MainButtonAttribute(self)
	...
	if second_cast ~= "" then
		f:SetAttribute("type2", "spell")
		f:SetAttribute("spell2", second_cast)
	end
...
function Necrosis:SpellstoneUpdateAttribute(nostone)
	...
	-- If in combat, do nothing (secure attributes cannot be changed)
	if InCombatLockdown() or not f then
		return
	end

	-- Detect if a spellstone is currently equipped (TBC: usually slot 18 or 17)
	local equippedSlot = nil
	local id18 = GetInventoryItemID("player", 18)
	local id17 = GetInventoryItemID("player", 17)
	if id18 and Necrosis.IsSpellStone and Necrosis.IsSpellStone(id18) then
		equippedSlot = 18
	elseif id17 and Necrosis.IsSpellStone and Necrosis.IsSpellStone(id17) then
		equippedSlot = 17
	end

	-- Case A: No spellstone in bags AND none equipped => left click creates
	if nostone and not equippedSlot then
		local castName = Necrosis.GetSpellCastName("spellstone")
		if castName then
			castName = castName:gsub("%s*%b()", ""):gsub("%s+$", "")
		end
		f:SetAttribute("type1", "spell")
	...
	-- Case B: Spellstone is equipped => left click USES it (puts it on cooldown)
	...
	-- Case C: Spellstone exists in bags but is not equipped => left click EQUIPS it
	-- (Do not /use here; the next click will /use once it's equipped.)
	if NecrosisConfig.ItemSwitchCombat[1] then
		local itemName = NecrosisConfig.ItemSwitchCombat[1]
		f:SetAttribute("type1", "macro")
		f:SetAttribute("macrotext1", "/stopcasting\n/equipslot 18 " .. itemName .. "\n/equipslot 17 " .. itemName)
	end
end
...
function Necrosis:FirestoneUpdateAttribute(nostone)
	...
	-- Un clic gauche crée la pierre
	if nostone then
		local castName = Necrosis.GetSpellCastName("firestone")
		if castName then
			castName = castName:gsub("%s*%b()", ""):gsub("%s+$", "")
		end
	...
	if NecrosisConfig.ItemSwitchCombat[2] then
		local itemName = NecrosisConfig.ItemSwitchCombat[2]
		f:SetAttribute("type1", "macro")
		-- TBC: equip the stone (slot 18 first, then 17), then /use for the on-use effect.
