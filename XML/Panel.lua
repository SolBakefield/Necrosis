...
function Necrosis:SetPanel(PanelID)
	...
	elseif PanelID == 6 then
		...
		HideUIPanel(NecrosisTimersConfig)
		--HideUIPanel(NecrosisOverlayConfig)
				if type(self.SetMiscConfig) == "function" then
			self:SetMiscConfig()
		else
			DEFAULT_CHAT_FRAME:AddMessage("|cffff0000Necrosis: SetMiscConfig() is missing (Options-Misc.lua not loaded before Panel.lua). Fix load order in Necrosis.toc so XML/Options-Misc.lua loads before XML/Panel.lua.|r")
			return
		end
