...
function Necrosis:AddFrame(FrameName,spellTexture)
	...
	-- Définition de ses textes
	-- Timer text (inside bar, right-justified)
	local FontString = frame:CreateFontString(FrameName .. "OutText", "OVERLAY", "TextStatusBarText")
	FontString:SetWidth(60)
	FontString:SetHeight(14)
	FontString:SetTextColor(1, 1, 1)
	FontString:ClearAllPoints()
	FontString:SetPoint("RIGHT", FrameName, "RIGHT", 6, 0)
	...
	FontString:SetJustifyV("MIDDLE")
	FontString:ClearAllPoints()
	FontString:SetPoint("LEFT", FrameName, "LEFT", 10, 0)
	FontString:Show()
...
function NecrosisUpdateTimer(tableau, Changement)
	...
	for index =  1, #tableau, 1 do
		...
		if PercentColor > 0.5 then -- red varient
			r = (1 - PercentColor) * 2 * (207/255)
			local t = (1 - PercentColor) * 2
    		g = 1 - t * (1 - (207/255))
		else -- green varient
			r = 1
